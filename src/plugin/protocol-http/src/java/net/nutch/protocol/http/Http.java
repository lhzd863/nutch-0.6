...
public class Http implements Protocol {
  ...
  static {
    if (NutchConf.getBoolean("http.verbose", false))
  ...
  private static final int MAX_REDIRECTS =
    NutchConf.getInt("http.redirect.max", 3);

  static String PROXY_HOST = NutchConf.get("http.proxy.host");
  static int PROXY_PORT = NutchConf.getInt("http.proxy.port",8080);
  static boolean PROXY = (PROXY_HOST != null && PROXY_HOST.length() > 0);
  
  static int TIMEOUT = NutchConf.getInt("http.timeout", 10000);
  ...
  static int MAX_THREADS_PER_HOST =
    NutchConf.getInt("fetcher.threads.per.host", 1);
