...
public class NDFS implements FSConstants {
    public static final Logger LOG = LogFormatter.getLogger("net.nutch.fs.NDFS");
    private static final long GIGABYTE = 1024 * 1024 * 1024;
    private static long numGigs = NutchConf.getLong("ndfs.datanode.maxgigs", 100);
     ...
     * @author Mike Cafarella
     **********************************************************/
    public static class NameNode extends net.nutch.ipc.Server {
        ...
        public static void main(String argv[]) throws IOException, InterruptedException {
            if (argv.length < 2) {
                System.out.println("NDFS$NameNode <port> <namespace_dir>");
                System.exit(-1);
            }
            ...
            NameNode namenode = new NameNode(dir, port);
            namenode.start();
            namenode.join();
        }
    }
     ...
     * @author Mike Cafarella
     **********************************************************/
    public static class DataNode extends net.nutch.ipc.Client {
        FSDataset data;
        String localName;
        ...
        public DataNode(String machineName, File dir, InetSocketAddress nameNodeAddr) throws IOException {
            ...
            new Thread(new DataXceiveServer(ss)).start();
        }
                    ...
                    for (int i = 0; i < blocks.length; i++) {
                        if (!data.isValidBlock(blocks[i])) {
                            ...
                            if (xferTargets[i].length > 0) {
                                ...
                                new Thread(new DataTransfer(xferTargets[i], blocks[i])).start();
                            }
                        }
                    }
                    break;
        ...
        class DataXceiveServer implements Runnable {
            ...
            public void run() {
                try {
                    while (true) {
                        Socket s = ss.accept();
                        new Thread(new DataXceiver(s)).start();
                    }
                } catch (IOException ie) {
        ...
        public static void main(String argv[]) throws IOException {
            if (argv.length < 3) {
                System.out.println("NDFS$DataNode <dataDir> <localMachine> <namenode:port>");
                System.exit(-1);
            }
            
            File dir = new File(argv[0]);
            String localMachine = argv[1];
            String nameNodeStr = argv[2];
            ...
            int nameNodePort = Integer.parseInt(nameNodeStr.substring(colon + 1));
            InetSocketAddress nameNodeAddr = new InetSocketAddress(nameNodeName, nameNodePort);

            DataNode datanode = new DataNode(localMachine, dir, nameNodeAddr);
            while (true) {
                try {
                    datanode.offerService();
                } catch (Exception ex) {
                    LOG.info("Lost connection to namenode [" + nameNodeAddr + "].  Retrying...");
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ie) {
                    }
